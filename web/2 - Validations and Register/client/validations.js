...
function makePredicates(predicateDefs) {
  ...
  for (let [predName, predValue] of Object.entries(predicateDefs)) {
    const predExpr = predValue.hasOwnProperty("expr")
      ? predValue.expr
      : predValue;
    let predDef =
      predExpr instanceof Function
        ? { expr: predExpr }
...
function installValidators(ancestorNode = document) {
  ...
  fieldsToValidate(ancestorNode).forEach((field) =>
    field.addEventListener("input", (ev) => validateField(field))
  );
}
...
function validateField(field) {
  ...
  if (!statusElem) {
    throw new ValidationError(
      `No status element for field: ${describeField(field)}!`
    );
  }
